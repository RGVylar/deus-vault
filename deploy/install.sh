#!/usr/bin/env bash
# Deus Vault LXC installer (Debian 12)
...
if [[ -n "$GITHUB_TOKEN" ]]; then
    REPO_URL="${REPO_URL:-https://${GITHUB_TOKEN}@github.com/RGVylar/deus-vault.git}"
else
    REPO_URL="${REPO_URL:-https://github.com/RGVylar/deus-vault.git}"
fi
REPO_BRANCH="${REPO_BRANCH:-main}"
APP_DIR="${APP_DIR:-/opt/deus-vault}"
APP_USER="${APP_USER:-deusvault}"
...
${GRN}✔ Deus Vault installed${NC}
  ...
  App dir:   $APP_DIR
  Logs:      journalctl -u deus-vault-backend -f

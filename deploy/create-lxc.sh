#!/usr/bin/env bash
# Create and configure a Deus Vault LXC on Proxmox
# Run this on the Proxmox host (not inside the LXC)
# Usage (on Proxmox host):
#   bash create-lxc.sh
#   CT_NAME=deusvault CT_ID=210 bash create-lxc.sh
...
msg "Cloning repo…"
pct exec "$CT_ID" -- rm -rf /tmp/deus-vault
pct exec "$CT_ID" -- git clone --branch main https://github.com/RGVylar/deus-vault.git /tmp/deus-vault

msg "Running install.sh…"
pct exec "$CT_ID" -- env DOMAIN="$DOMAIN" BACKEND_PORT="$BACKEND_PORT" bash /tmp/deus-vault/deploy/install.sh

echo ""
cat <<EOF

${GRN}✔ Deus Vault LXC ready${NC}
...
Logs:
  pct exec $CT_ID -- journalctl -u deus-vault-backend -f
